...
在`md5Upload.vue`文件中，MD5计算是对整个文件进行的，而不是对单个分片分别计算。具体实现如下：
...
虽然文件是分片读取的，但所有分片的数据都被添加到同一个`SparkMD5`实例中，最后通过`spark.end()`方法获取整个文件的哈希值。这样可以：
1. 避免一次性读取大文件导致内存溢出
2. 保证获得的是整个文件的完整MD5值
3. 实现文件的唯一性校验
...
- 通过`pauseUpload`和`resumeUpload`函数实现
- 记录当前上传的分片索引`currentChunkIndex.value`
- 暂停后可以从该位置继续上传
- 记录暂停时间，计算真实上传时长
...
1. **MD5计算**：对整个文件进行计算，而不是对单个分片单独计算，保证文件唯一性
2. **断点续传**：通过记录已上传分片实现，支持暂停/继续功能
3. **秒传**：通过比对完整文件的MD5哈希值实现，避免重复上传
4. **性能优化**：
   - 分片读取避免内存溢出
   - 支持并发上传提高效率
   - 精确的上传进度和速度计算